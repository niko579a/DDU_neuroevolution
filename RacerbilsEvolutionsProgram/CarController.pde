class CarController {
  ...
  void update() {
    ...
    if(sensorSystem.lapCompledted){
      if(bestLab > sensorSystem.lapTimeInFrames){
        bestWeights = hjerne.weights;
        bestBiases = hjerne.biases;
        bestLab = sensorSystem.lapTimeInFrames;
      }
      generation ++;
